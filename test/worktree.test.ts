...
describe("worktree", () => {
  ...
  describe("concurrent isolation", () => {
    it("two worktrees from same repo have distinct paths", () => {
      const wt1 = createWorktree(repoDir, "concurrent-a")!;
      const wt2 = createWorktree(repoDir, "concurrent-b")!;

      expect(wt1).toBeDefined();
      expect(wt2).toBeDefined();
      expect(wt1.path).not.toBe(wt2.path);
      expect(wt1.branch).toBe("pi-agent-concurrent-a");
      expect(wt2.branch).toBe("pi-agent-concurrent-b");

      // Both should have the repo's files
      expect(existsSync(join(wt1.path, "README.md"))).toBe(true);
    ...
    it("changes in one worktree are invisible to the other", () => {
      const wt1 = createWorktree(repoDir, "iso-a")!;
      const wt2 = createWorktree(repoDir, "iso-b")!;

      // Make a change in wt1
      writeFileSync(join(wt1.path, "only-in-wt1.txt"), "this is only in worktree A");
      // Make a different change in wt2
      writeFileSync(join(wt2.path, "only-in-wt2.txt"), "this is only in worktree B");

      // wt2 should NOT see wt1's file
      expect(existsSync(join(wt2.path, "only-in-wt1.txt"))).toBe(false);
      // wt1 should NOT see wt2's file
      expect(existsSync(join(wt1.path, "only-in-wt2.txt"))).toBe(false);

      // Each should only see its own file
      expect(existsSync(join(wt1.path, "only-in-wt1.txt"))).toBe(true);
      expect(existsSync(join(wt2.path, "only-in-wt2.txt"))).toBe(true);
    ...
    it("cleanup of one worktree does not affect the other", () => {
      const wt1 = createWorktree(repoDir, "cleanup-a")!;
      const wt2 = createWorktree(repoDir, "cleanup-b")!;

      // Make changes in both
      writeFileSync(join(wt1.path, "file-a.txt"), "change in A");
      writeFileSync(join(wt2.path, "file-b.txt"), "change in B");

      // Cleanup wt1
      const result1 = cleanupWorktree(repoDir, wt1, "cleanup A");
      expect(result1.hasChanges).toBe(true);
      expect(result1.branch).toBe("pi-agent-cleanup-a");

      // wt2 should still exist and have its file
      expect(existsSync(wt2.path)).toBe(true);
      expect(existsSync(join(wt2.path, "file-b.txt"))).toBe(true);

      // wt1's path should be gone
      expect(existsSync(wt1.path)).toBe(false);

      // Cleanup wt2 as well
      const result2 = cleanupWorktree(repoDir, wt2, "cleanup B");
      expect(result2.hasChanges).toBe(true);
      expect(result2.branch).toBe("pi-agent-cleanup-b");
